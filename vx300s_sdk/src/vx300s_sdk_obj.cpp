...
void InterbotixRobotXS::robot_torque_enable(std::string const &cmd_type, std::string const &name, bool const &enable)
{
  if (cmd_type == "group" && group_map.count(name) > 0)
  {
    ...
    if (enable)
      ROS_INFO("[xs_sdk] The '%s' group was torqued on.", name.c_str());
    else
      ROS_INFO("[xs_sdk] The '%s' group was torqued off.", name.c_str());
  }
  else if (cmd_type == "single" && motor_map.count(name) > 0)
  {
    dxl_wb.torque(motor_map[name].motor_id, enable);
    if (enable)
      ROS_INFO("[xs_sdk] The '%s' joint was torqued on.", name.c_str());
    else
      ROS_INFO("[xs_sdk] The '%s' joint was torqued off.", name.c_str());
  }
  else if (cmd_type == "group" && group_map.count(name) == 0 || cmd_type == "single" && motor_map.count(name) == 0)
    ROS_WARN("[xs_sdk] The '%s' joint/group does not exist. Was it added to the motor config file?", name.c_str());
  else
...
bool InterbotixRobotXS::robot_get_motor_configs(void)
{
  ...
  if (ros::param::has("~port"))
  {
    ...
    ROS_ERROR("[vx300s_sdk] port private param was not found. Shutting down...");
    return false;
  }
...
{
  if (pub_states)
    pub_joint_states = node.advertise<sensor_msgs::JointState>(prefix + js_topic, 1);
}
...
bool InterbotixRobotXS::robot_srv_get_robot_info(interbotix_xs_sdk::RobotInfo::Request &req, interbotix_xs_sdk::RobotInfo::Response &res)
{
  ...
  for (auto &name : res.joint_names)
  {
    ...
    if (urdf_exists)
    {
      ptr = model.getJoint(prefix + name); // CHANGED TO ADD PREFIX
