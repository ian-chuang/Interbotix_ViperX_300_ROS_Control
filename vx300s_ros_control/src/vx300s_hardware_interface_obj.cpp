...
void XSHardwareInterface::init()
{
  ...
  ros::param::get("~loop_hz", loop_hz);
  ros::param::get("~group_name", group_name);
  ros::param::get("~gripper_name", gripper_name);
  ...
  sub_joint_states = nh.subscribe(prefix + "joint_states", 1, &XSHardwareInterface::joint_state_cb, this);
  ...
  for (int i = 0; i < num_joints; ++i) {
    std::string joint_name = prefix + joint_names.at(i);

     // Create joint state interface
    hardware_interface::JointStateHandle jointStateHandle(joint_name, &joint_positions.at(i), &joint_velocities.at(i), &joint_efforts.at(i));
    joint_state_interface.registerHandle(jointStateHandle);

    joint_limits_interface::JointLimits limits;
    ptr = model.getJoint(joint_name);
    getJointLimits(ptr, limits);
    getJointLimits(joint_name, nh, limits);
